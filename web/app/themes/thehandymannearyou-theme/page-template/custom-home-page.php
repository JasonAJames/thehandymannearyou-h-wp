	...
	<?php if( get_theme_mod('lz_cleaning_services_slider_hide_show') != ''){ ?>
		<section id="slider">
		  	<div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel"> 
			    <?php $lz_cleaning_services_slider_pages = array();
		      	for ( $count = 1; $count <= 4; $count++ ) {
			        $mod = intval( get_theme_mod( 'lz_cleaning_services_slider' . $count ));
			        if ( 'page-none-selected' != $mod ) {
			          $lz_cleaning_services_slider_pages[] = $mod;
			        }
		      	}
		      	if( !empty($lz_cleaning_services_slider_pages) ) :
			        $args = array(
			          	'post_type' => 'page',
			          	'post__in' => $lz_cleaning_services_slider_pages,
			          	'orderby' => 'post__in'
			        );
			...
			<div class="container">
				<div class="service-box">
					<?php if( get_theme_mod('cleaning_features_title') != ''){ ?>
			    		<h2><?php echo esc_html(get_theme_mod('cleaning_features_title','')); ?></h2>
			    		<hr>
			    	<?php }?>
		            <div class="row">
			      		<?php $lz_cleaning_services_catData1=  get_theme_mod('cleaning_features_cat'); 
			      		if($lz_cleaning_services_catData1){ 
			      			$page_query = new WP_Query(array( 'category_name' => esc_html($lz_cleaning_services_catData1 ,'lz-cleaning-services')));?>
