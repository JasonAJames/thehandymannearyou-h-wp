...
function lz_cleaning_services_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting('lz_cleaning_services_show_site_title',array(
       'default' => true,
       'sanitize_callback'	=> 'lz_cleaning_services_sanitize_checkbox'
    ));
    $wp_customize->add_control('lz_cleaning_services_show_site_title',array(
       'type' => 'checkbox',
       'label' => __('Show / Hide Site Title','lz-cleaning-services'),
       'section' => 'title_tagline'
    ));

    $wp_customize->add_setting('lz_cleaning_services_show_tagline',array(
       'default' => true,
       'sanitize_callback'	=> 'lz_cleaning_services_sanitize_checkbox'
    ));
    $wp_customize->add_control('lz_cleaning_services_show_tagline',array(
       'type' => 'checkbox',
       'label' => __('Show / Hide Site Tagline','lz-cleaning-services'),
       'section' => 'title_tagline'
    ));
	...
	$wp_customize->add_setting('lz_cleaning_services_call_number',array(
		'default'=> '',
		'sanitize_callback'	=> 'lz_cleaning_services_sanitize_phone_number'
	));	
...
final class LZ_Cleaning_Services_Customize {
	...
	public function sections( $manager ) {
		...
		$manager->add_section(
			new LZ_Cleaning_Services_Customize_Section_Pro(
				$manager,
				'lz_cleaning_services_example_1',
				array(
					'priority' => 9,
					'title'    => esc_html__( 'Cleaning Services Pro ', 'lz-cleaning-services' ),
					'pro_text' => esc_html__( 'Go Pro','lz-cleaning-services' ),
					'pro_url'  => esc_url( 'https://www.luzuk.com/products/cleaning-services-wordpress-theme/' ),
				)
			)
		);
	}
	...
	public function enqueue_control_scripts() {
		...
		wp_enqueue_script( 'lz-cleaning-services-customize-controls', trailingslashit( esc_url(get_template_directory_uri()) ) . '/assets/js/customize-controls.js', array( 'customize-controls' ) );

		wp_enqueue_style( 'lz-cleaning-services-customize-controls', trailingslashit( esc_url(get_template_directory_uri()) ) . '/assets/css/customize-controls.css' );
	}
}
